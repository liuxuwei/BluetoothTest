 ...
 * {@link com.qualcomm.qti.libraries.gaia.GAIA.Transport#BLE BLE} and
 * {@link com.qualcomm.qti.libraries.gaia.GAIA.Transport#BR_EDR BR_EDR}. Depending on the transport the structure of a GAIA
...
public abstract class GaiaManager {
     ...
     * {@link com.qualcomm.qti.libraries.gaia.GAIA.Status#NOT_SUPPORTED NOT_SUPPORTED}.</p>
     */
    ...
    protected void createRequest(GaiaPacket packet) {
        if (mShowDebugLogs) {
            Log.d(TAG, "Received request to send a packet with expected acknowledgement for command: "
                    + GaiaUtils.getGAIACommandToString(packet.getCommand()));
        }
            ...
            case GaiaRequest.Type.SINGLE_REQUEST:
                try {
                    ...
                    if (!sendGAIAPacket(bytes)) {
                        cancelTimeOutRequestRunnable(request.packet.getCommand());
                        Log.w(TAG, "Fail to send GAIA packet for GAIA command: "
                                + GaiaUtils.getGAIACommandToString(request.packet.getCommandId()));
    ...
    /**
     * <p>To manage any packet which had not been sent: the call to {@link #sendGAIAPacket(byte[]) sendGAIAPacket} has
     * returned false.</p>
     *
     * @param packet
     *          The packet which has not been sent.
     */
    protected abstract void onSendingFailed(GaiaPacket packet);
